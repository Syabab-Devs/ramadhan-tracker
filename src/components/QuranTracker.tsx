...
import { format, parseISO, isValid, isFuture, startOfDay, differenceInDays, addDays } from 'date-fns';
import { getDailyQuranProgress, saveQuranProgress, getAllQuranProgress, getSetting, DEFAULT_QURAN_PAGES, type QuranProgress } from '../lib/db';
import { BookOpen, Target, Calendar, ChevronLeft, ChevronRight, Award, BookMarked, AlertCircle, Calculator } from 'lucide-react';
...
export default function QuranTracker() {
  ...
  const { data: totalProgress } = useQuery({
    ...
    queryFn: async () => {
      ...
      const total = allProgress.reduce((acc, curr) => ({
  ...
  const calculateDailyTarget = (targetKhatam: number) => {
    try {
      ...
      const endDate = ramadhanEndDate ? parseISO(ramadhanEndDate) : addDays(today, 30);
      ...
      const remainingPages = Math.max(0, totalPagesNeeded - (totalProgress?.pagesRead || 0));
      const remainingJuz = Math.max(0, totalJuzNeeded - (totalProgress?.juzRead || 0));
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="mb-6 bg-emerald-50 p-4 rounded-lg">
          <div className="flex items-center gap-3 text-emerald-700">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{totalProgress?.khatamCount || 0}x Khatam</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between text-sm text-gray-600 mb-2">
            ...
            <span>{totalProgress?.pagesRead || 0}/{totalPages} Pages</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-emerald-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 text-emerald-700 mb-2">
              <BookOpen size={20} />
              <span className="font-medium">{t('quranTracker.totalPagesRead')}</span>
            </div>
            <p className="text-2xl font-bold text-emerald-800">{totalProgress?.pagesRead || 0}</p>
          </div>
          <div className="bg-emerald-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 text-emerald-700 mb-2">
              <Target size={20} />
              <span className="font-medium">{t('quranTracker.totalJuzRead')}</span>
            </div>
            <p className="text-2xl font-bold text-emerald-800">{totalProgress?.juzRead || 0}/30</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              name="target"
              value={target}
              onChange={(e) => mutation.mutate({ ...dayProgress, target: Number(e.target.value) })}
            ...
            <div className="mt-3 bg-emerald-50 p-4 rounded-lg">
              ...
              <div className="space-y-2 text-sm text-emerald-800">
                <p>To complete {target}x Khatam in {suggestion.daysLeft} days:</p>
                <ul className="list-disc list-inside pl-2">
                  <li>Still need to read {suggestion.remainingPages} pages ({suggestion.pages} pages per day)</li>
                  <li>Still need to complete {suggestion.remainingJuz} juz ({suggestion.juz} juz per day)</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">{t('quranTracker.currentJuzHelp')}</p>
          </div>
