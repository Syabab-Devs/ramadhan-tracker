...
import { format, parseISO, subDays, isSameDay, isAfter, isBefore, addDays } from 'date-fns';
...
export default function IbadahHistory() {
  ...
  const { data: allHistory } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allIbadah.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }
  });
  ...
  const history = React.useMemo(() => {
    ...
    const startDate = timePeriod === 'all' 
  ...
  const calculateStreak = () => {
    ...
    for (let i = 0; i < allHistory.length; i++) {
      ...
      if (format(date, 'yyyy-MM-dd') !== format(expectedDate, 'yyyy-MM-dd')) break;
      
      const allPrayers = allHistory[i].fajr && allHistory[i].dhuhr && 
                        allHistory[i].asr && allHistory[i].maghrib && 
                        allHistory[i].isha && allHistory[i].tarawih;
  ...
  const calculateCompletionRate = () => {
    ...
    const completedDays = history.filter(day => 
      day.fajr && day.dhuhr && day.asr && 
      day.maghrib && day.isha && day.tarawih
    ).length;
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold">{t('dailyIbadah.history')}</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          <div className="bg-emerald-50 p-4 rounded-lg">
            <div className="flex items-center gap-3 text-emerald-700">
              ...
              <div>
                <h3 className="font-semibold">{t('dailyIbadah.currentStreak')}</h3>
                <p className="text-2xl font-bold">{calculateStreak()} {t('dailyIbadah.daysStreak')}</p>
              </div>
            </div>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {history.map((day) => (
                  <tr key={day.date} className="hover:bg-gray-50">
                    ...
                    {['fajr', 'dhuhr', 'asr', 'maghrib', 'isha', 'tarawih', 'tahajjud', 'dhikr', 'charity'].map((activity) => (
